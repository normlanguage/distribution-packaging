...
Fedora 44 builds and installed-package validation passed. Maven reported 4,531 tests with zero failures, zero errors and one upstream-disabled usage example. Rawhide mock validation is pending. Rpmlint reports the generated JDK license address, documentation size and missing separate check-section findings; tests run during Maven's build lifecycle.
